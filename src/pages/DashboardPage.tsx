...
export default function DashboardPage() {
  ...
  const weeklyTrend = useMemo(() => {
    const weekGoals = goals.filter((g) => g.periodType === 'week' && g.achievedAmount !== null);
    const buckets = new Map<string, { label: string; sum: number; count: number; order: string }>();
    for (const g of weekGoals) {
      const key = weekBucketKey(g.startDate);
      const { percentage } = computeGoal(g);
      if (percentage === null) continue;
      const bucket = buckets.get(key) ?? { label: weekBucketLabel(g.startDate), sum: 0, count: 0, order: key };
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Card className="p-5 lg:col-span-2">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="w-4 h-4 text-bordeaux" />
            <h3 className="font-display font-bold text-ink">تطور معدل الإنجاز الأسبوعي</h3>
          </div>
          {weeklyTrend.length === 0 ? (
            <EmptyState title="لا توجد بيانات كافية بعد" description="ستظهر هنا نسب الإنجاز الأسبوعية بعد تسجيل الإنجازات." />
          ) : (
            <div dir="ltr" className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={weeklyTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                  ...
                  />
                  <Bar dataKey="نسبة" fill={CHART_COLORS.bordeaux} radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
